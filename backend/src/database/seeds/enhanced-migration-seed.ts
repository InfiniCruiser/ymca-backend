...
// OEA Data structure from the sample files
interface OEAMetric {
  name: string;
  points: number;
  maxPoints: number;
...
interface OEAPerformanceCategory {
  category: string;
  metrics: OEAMetric[];
  totalPoints: number;
  maxPoints: number;
...
interface OEAPerformanceSnapshot {
  operationalPerformance: OEAPerformanceCategory;
  financialPerformance: OEAPerformanceCategory;
}
...
interface OEAData {
  ...
  totalPoints: number;
  maxPoints: number;
  performanceSnapshot: OEAPerformanceSnapshot;
  overallSupportDesignation: string;
}
...
export class EnhancedDatabaseSeeder {
  ...
  async seed() {
    ...
    try {
      ...
      // Seed performance calculations with OEA data structure
    ...
    const users = [
      {
        ...
        firstName: 'Admin',
        lastName: 'User',
        organizationId: organizations[0].id, // XYZ YMCA
      ...
      },
      {
        ...
        firstName: 'St. Louis',
        lastName: 'Admin',
        organizationId: organizations[1].id, // St. Louis Y
        role: 'ASSOCIATION_ADMIN',
      ...
      },
      {
        ...
        firstName: 'John',
        lastName: 'Doe',
        organizationId: organizations[2].id, // Charlotte YMCA
        role: 'PROGRAM_OWNER',
        programAreas: ['Child Protection', 'Governance'],
        locations: ['Main Branch'],
        isActive: true
      },
      {
        ...
        firstName: 'Jane',
        lastName: 'Smith',
        organizationId: organizations[3].id, // LA YMCA
        role: 'ASSOCIATION_ADMIN',
        programAreas: ['Risk Mitigation', 'Engagement'],
        locations: ['Downtown Branch'],
        isActive: true
      },
      {
        ...
        firstName: 'Mike',
        lastName: 'Johnson',
        organizationId: organizations[4].id, // Chicago YMCA
        role: 'BOARD_LIAISON',
  ...
  private async seedOEAPerformanceCalculations(organizations: Organization[], submissions: Submission[]) {
    ...
    // Define OEA data structure for each organization
    const oeaData: Record<string, OEAData> = {
      'XYZ': {
        id: 'xyz',
        name: 'XYZ YMCA',
        totalPoints: 34,
    ...
    // Create performance calculations for each organization
    for (const organization of organizations) {
      ...
      for (const submission of orgSubmissions) {
        // Use OEA data if available, otherwise calculate from submissions
  ...
  private createOEAFromData(oeaData: OEAData, period: string) {
  ...
  private calculatePerformanceFromSubmission(submission: Submission) {
    ...
    // Calculate scores based on responses (for organizations without OEA data)
    ...
    if (responses['RM.CP.001'] === 'Yes') riskMitigationScore += 2;
    if (responses['RM.CP.002'] === 'Yes') riskMitigationScore += 2;
    if (responses['RM.CP.003'] === 'Yes') riskMitigationScore += 2;

    // Calculate governance score
    if (responses['GV.BO.001'] === 'Yes') governanceScore += 3;
    if (responses['GV.BO.002'] === 'Yes') governanceScore += 3;
    if (responses['GV.BO.003'] === 'Yes') governanceScore += 3;

    // Calculate engagement score
    if (responses['EG.ME.001'] === 'Yes') engagementScore += 2;
    if (responses['EG.ME.002'] === 'Yes') engagementScore += 2;
    if (responses['EG.ME.003'] === 'Yes') engagementScore += 2;

    // Add some variation for other metrics
    membershipGrowthScore = Math.floor(Math.random() * 4) + 1; // 1-4 points
    staffRetentionScore = Math.floor(Math.random() * 4) + 1; // 1-4 points
    graceScore = Math.floor(Math.random() * 4) + 1; // 1-4 points

    // Financial metrics (simulated for non-OEA organizations)
