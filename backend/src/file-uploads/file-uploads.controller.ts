...
export class FileUploadsController {
  ...
  @Get('upload-status')
  @ApiOperation({ 
    summary: 'Get upload status (alternative endpoint)',
    description: 'Alternative endpoint for upload progress to bypass validation issues'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async getUploadStatus(
    ...
    console.log('=== UPLOAD STATUS ENDPOINT DEBUG ===');
    console.log('Timestamp:', new Date().toISOString());
    console.log('Request URL:', req.url);
    console.log('Query params:', req.query);
    console.log('=== END DEBUG ===');
